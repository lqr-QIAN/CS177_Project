...
def load_dataloader(n_fold,args):
    ...
    """
    tr_data_items = osp.join(data_root, f'split/{n_fold}_fold_tr_items_dose_matched.npy')
    val_data_items = osp.join(data_root, f'split/{n_fold}_fold_val_dose_matched.npy')
    test_data_items = osp.join(data_root, f'split/{n_fold}_fold_test_dose_matched.npy')

    # 加载训练数据并限制为前10000条
    tr_items_raw = np.load(tr_data_items, allow_pickle=True)
    tr_items_small = tr_items_raw[:70000]
    ...
    tr_dataloader = DataLoader(tr_dataset[:70000], batch_size=args.batch_size, shuffle=False, num_workers=4, drop_last=True)
    val_dataloader = DataLoader(val_dataset[:16000], batch_size=args.batch_size, shuffle=False, num_workers=4, drop_last=True)
    test_dataloader = DataLoader(test_dataset[:17800], batch_size=args.batch_size, shuffle=False, num_workers=4, drop_last=True)
...
def infer(model, dataloader, device, args=None):
    ...
    y_pred = torch.cat(y_pred, dim=0).cpu().detach().numpy()
    mse, rmse, mae, r2, pearson, spearman = get_metrics(y_true, y_pred)
    print('Infer result: mse:{:.4f} rmse:{:.4f} mae:{:.4f} r2:{:.4f} pearson:{:.4f} spearman:{:.4f}'.format(mse, rmse, mae, r2, pearson, spearman))
