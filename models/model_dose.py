...
class SynergyxNet(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # 如果使用 Geneformer，调整基因数量以减少显存使用
        if self.args.use_geneformer:
            # 限制最大基因数量为 1000，这应该能显著减少内存使用
            #self.genes_nums = min(self.genes_nums, 1000)
            print(f"Using Geneformer with {self.genes_nums} genes")
